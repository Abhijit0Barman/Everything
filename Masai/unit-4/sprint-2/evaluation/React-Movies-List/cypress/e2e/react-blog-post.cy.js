...
data.forEach(({ submission_link: url, id, json_server_link: server_url }) => {
  describe("react-blog-posts", function () {
    ...
    it("Show Loading indicator between req and res from the server",()=>{
      cy.intercept("GET","**/movies",(req,res)=>{
        req.reply((res)=>{
          res.delay=2500
        })        
      }).as("getMovies")
      cy.visit(url);
      cy.get(`[data-testid="loading-indicator"]`).should("have.text","Loading...");
      cy.get(`[data-testid="movie-container"]`).should("not.exist");    
      cy.wait("@getMovies").then(()=>{
        cy.get(`[data-testid="loading-indicator"]`).should("not.exist");
        cy.get(`[data-testid="movie-container"]`).should("exist");
      })      
      cy.then(() => {
        acc_score += 1;
      });
