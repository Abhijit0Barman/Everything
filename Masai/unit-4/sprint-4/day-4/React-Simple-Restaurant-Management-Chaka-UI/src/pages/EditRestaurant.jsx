...
import { getSingleRestaurant, putRestaurant } from '../utils/api';
...
const EditRestaurant = () => {
  ...
  const [state, dispatch] = useReducer(editRestaurantReducer, initialState);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const restaurant = await getSingleRestaurant(id);
        dispatch({ type: 'UPDATE_STATE', payload: restaurant });
      } catch (error) {
        console.log(error);
      }
    };
