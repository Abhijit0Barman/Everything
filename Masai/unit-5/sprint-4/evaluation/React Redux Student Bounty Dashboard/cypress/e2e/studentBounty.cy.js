  ...
  describe("Student Bounty", () => {
    ...
    it(`On filtering the data in redux store should also be filtered accordingly`, () => {
      ...
      cy.wait(2000).then(() => {
        ...
        cy.get("[data-testid=batch-web101]").click({ force: true });
        cy.get("[data-testid=batch-js201]").click({ force: true });
        cy.wait(2000).then(() => {
          cy.window()
            .then(getStudentStore)
            .its("students")
    ...
    it(`On filtering the the UI on home page should update accordingly`, () => {
      ...
      cy.wait(2000).then(() => {
        ...
        cy.get("[data-testid=batch-rct211]").click({ force: true });
        cy.get("[data-testid=batch-nxm101]").click({ force: true });
        cy.wait(2000).then(() => {
          cy.get(".student-batch").then((res) => {
            for (let el of res) {
              console.log(el);
